...
class Simulation:
    def __init__(self, n_particles=10) -> None:
        pg.init()
        pg.mixer.init()
    ...
    def handle_collisions(self):
        ...
        for i, p in enumerate(self.particles):
            for other in self.particles[i+1:]:
                if p.overlaps(other): self.elastic_collisions(p, other)
    ...
    def elastic_collisions(self, p1, p2):
        ...
        n_v1 = v1 - 2*m2 / M * pg.math.Vector2.dot(v1 - v2, x1 - x2) * (x1 - x2) / d
        n_v2 = v2 - 2*m1 / M * pg.math.Vector2.dot(v2 - v1, x2 - x1) * (x2 - x1) / d
        p1.vel = n_v1
        p2.vel = n_v2
    ...
    def display(self):
        ...
        self.screen.blit(pygame.font.SysFont("Calibri", 35).render(f"Collisions: {self.n_collisions}", 
                         True, 
                         WHITE), 
                         (5, 5))
        for p in self.particles:
